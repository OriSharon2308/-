...
const OPENING_TEMPLATES = [
  "שלום {name}! איזה כיף שבאת. זה שיעור מספר {n} שלנו — היום נלמד על {topic}. מוכן/ה להתחיל?",
  "היי {name}! מוכן/ה לשיעור מספר {n}? היום מחכה לנו משהו מגניב — {topic}. בוא/י נתחיל!",
  "שלום {name}! טוב לראות אותך שוב. בשיעור {n} של היום נגלה יחד את {topic}. מוכן/ה?",
  "היי {name}, ברוך/ה הבא/ה לשיעור מספר {n}! היום נצא להרפתקה בנושא {topic}. יאללה, מתחילים?",
  "שלום {name}! אני כבר מחכה לשיעור הזה — מספר {n}. היום נלמד על {topic}, ונראה לי שתאהב/י את זה. מוכן/ה?",
  "היי {name}! הגיע הזמן לשיעור {n} שלנו. הנושא של היום: {topic}. בוא/י נראה כמה את/ה חזק/ה!",
  "שלום {name}! שיעור מספר {n} מתחיל עכשיו. היום נבין ביחד את {topic} — צעד אחר צעד. מוכן/ה?",
  "היי {name}, כיף שחזרת! בשיעור {n} נלמד על {topic}. וזה הולך להיות מעניין. מתחילים?",
  "שלום {name}! זה כבר שיעור מספר {n} — את/ה מתקדם/ת יפה! היום: {topic}. בוא/י נצלול פנימה!",
  "היי {name}! מוכן/ה לעוד שיעור מעולה? מספר {n} — והפעם נכיר את {topic}. קדימה!",
];
...
function buildOpeningGreeting({ name, topic, userId, grade }) {
  ...
  const who = name || "אלוף/ה"; // genderize ימיר לפי מין הילד
...
// ה-system מוחזר כשני בלוקים:
//   [0] קבוע לחלוטין (כללים + קטלוג הכלים) — מסומן cache:true. זהה תו-בתו לכל
//       ילד ולכל נושא, ולכן נכתב למטמון פעם אחת ומשם רק נקרא (~10% מהמחיר).
//   [1] משתנה (מגדר, שם, נושא, פרופיל) — בלי מטמון. קצר, ולכן זול במחיר מלא.
// לפני הפיצול הכל היה בלוק אחד עם שם-הילד בתוכו: כל ילד יצר prefix אחר, המטמון
// נכתב מחדש כמעט בכל קריאה, ו-65% מעלות הסוכן הלכה על cache-write בלבד.
...
function buildSystem({ gender, profileText, topic, name }) {
  ...
  // כל מה שמשתנה בין ילד לילד — מחוץ למטמון
  const varLines = [
    `דבר/י בלשון ${fem ? "נקבה" : "זכר"}, בלי לוכסנים, בלי markdown, בלי כוכביות.`,
  ];
  if (name) varLines.push(`שם הילד: ${name}. פנה/י אליו בשמו — בפתיחה, בשבחים, וברגעים חמים (לא בכל משפט).`);
  if (topic) varLines.push(`נושא הלמידה כרגע: ${topic}. בחר/י מהמאגר את הויזואל המתאים לנושא והצג/י אותו מיוזמתך.`);
  if (profileText) varLines.push(`\n${profileText}`);
