...
const CURRICULUM = {
  1: {
    ...
    topics: [
      ...
      { key: "חיבור עד 20", gen: "addition", levels: [1, 2, 3], params: { max: 20 } },
      { key: "חיסור עד 20", gen: "subtraction", levels: [1, 2, 3], params: { max: 20 } },
      { key: "חיבור וחיסור עד 100", gen: "addSub100", levels: [3, 4, 5], params: { max: 100 } },
      {
        ...
        sub: [
          ...
          { key: "ציור צורות", gen: "shapeCreate", levels: [1, 2, 3] },
        ],
      },
...
function gradeToNum(label) {
  return GRADE_LABEL_TO_NUM[String(label).trim()] || null;
}
...
function topicsForApi(gradeNum) {
  ...
  return g.topics.map((t) => ({
    key: t.key,
    sub: Array.isArray(t.sub) ? t.sub.map((s) => s.key) : [],
  }));
}
