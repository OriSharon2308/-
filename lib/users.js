...
function validateRegistration(input) {
  ...
  const gender = String(input.gender ?? "").trim() === "female" ? "female" : "male";
  ...
  return {
    ok: errors.length === 0,
    errors,
    value: { username, email, password, school, grade, age, gender },
  };
}
...
function publicUser(record) {
  ...
  for (const k of ["firstName", "lastName"]) {
    if (record[k]) out[k] = record[k];
  }
  return out;
}

/* ---------- אדמין: צפייה מלאה + עריכה ---------- */

/** רשומה מלאה לאדמין — כל השדות חוץ מ-passwordHash. */
...
/** כל המשתמשים (לאדמין) — מערך רשומות בלי passwordHash. */
function getAllUsers() {
  const users = loadUsers();
  return Object.values(users).map(adminView);
}

/** רשומת משתמש מלאה לאדמין לפי id (בלי passwordHash). */
function getUserAdmin(id) {
  const users = loadUsers();
  return users[id] ? adminView(users[id]) : null;
}

const EDITABLE_FIELDS = new Set([
  "firstName", "lastName", "dob", "parentName", "parentPhone", "parentEmail",
  "email", "age", "grade", "school", "gender", "notes",
]);

/** עדכון שדות פרופיל ע"י אדמין. מחזיר את הרשומה המעודכנת (בלי hash) או null. */
function updateUserFields(id, patch) {
  const users = loadUsers();
  const record = users[id];
  if (!record) return null;
  for (const [k, v] of Object.entries(patch || {})) {
    ...
    if (v === null || v === undefined || v === "") {
      if (k !== "age" && k !== "gender") delete record[k];
      continue;
    }
    if (k === "age") {
      const n = Number.parseInt(v, 10);
      if (Number.isFinite(n) && n >= 1 && n <= 120) record.age = n;
      continue;
    }
    if (k === "gender") {
      record.gender = String(v) === "female" ? "female" : "male";
      continue;
    }
    record[k] = String(v).slice(0, 300);
  }
...
/** קביעת סיסמה חדשה לתלמיד (ע"י אדמין). מאחסן hash בלבד. */
function setPassword(id, newPassword) {
  const pw = String(newPassword ?? "");
  if (pw.length < 4) return { ok: false, error: "סיסמה חייבת להיות לפחות 4 תווים." };
  const users = loadUsers();
  const record = users[id];
  if (!record) return { ok: false, error: "המשתמש לא נמצא." };
  record.passwordHash = hashPassword(pw);
  record.updatedAt = new Date().toISOString();
  users[id] = record;
  saveUsers(users);
  return { ok: true };
