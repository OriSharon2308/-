...
/**
 * נתיב קובץ-הזהב. מכיתה ב׳ ומעלה — תיקייה לכל כיתה (golden/grade-3/…), כי אותו
 * שם-נושא חוזר בכיתות שונות ("זמן", "חקר נתונים") והקבצים היו דורסים זה את זה.
 * כיתה א׳ / בלי כיתה — נשארת בשורש (תאימות לאחור לשיעורים הקיימים).
 */
function fileFor(topic, n, grade) {
  const slug = String(topic).trim().replace(/[^\w֐-׿-]+/g, "_").slice(0, 60);
  const g = +grade;
  const dir = g >= 2 && g <= 12 ? path.join(GOLDEN_DIR, `grade-${g}`) : GOLDEN_DIR;
  return path.join(dir, `${slug}#${Math.max(1, +n || 1)}.json`);
}

/** מערך-הזהב של השיעור — או null אם אין. */
function get(topic, n, grade) {
  if (!topic) return null;
  const g = readJson(fileFor(topic, n, grade), null);
 ...
 */
function screens(topic, n, phaseName, grade) {
  const g = get(topic, n, grade);
...
function phase(topic, n, phaseName, screenIdx, grade) {
  const scr = screens(topic, n, phaseName, grade);
  if (!scr) return null;
...
function save(topic, n, data, grade) {
  ...
  try {
    fs.mkdirSync(path.dirname(fileFor(topic, n, grade)), { recursive: true });
    const file = fileFor(topic, n, grade);
...
function stagesFor(topic, n, grade) {
  const g = get(topic, n, grade);
