...
function emptyTopic() {
  return {
    ...
    // ארבע הטעויות האחרונות בנושא: הטקסט, התשובה הנכונה ומה שהילד ענה.
    // זה הסיגנל שממנו lib/targeted-questions בונה שאלה בסגנון שהילד נופל בו —
    // סטטוס "מתקשה" לבדו אומר *שהוא* מתקשה, לא *במה*.
    misses: [],
...
function record(userId, ev = {}) {
  ...
  if (ev.correct === true) {
    ...
    if (typeof ev.text === "string" && ev.text.trim()) {
      if (!Array.isArray(t.misses)) t.misses = [];
      t.misses.push({
        text: ev.text.trim().slice(0, 120),
        answer: ev.answer == null ? "" : String(ev.answer).slice(0, 20),
        given: ev.given == null ? "" : String(ev.given).slice(0, 20),
        at: new Date().toISOString(),
      });
      if (t.misses.length > 4) t.misses = t.misses.slice(-4); // חלון קצר — רק המגמה העכשווית
    }
  }
