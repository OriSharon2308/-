...
function solidsCount(level = 1, p = {}) {
  ...
  const pool = lv === 1 ? SOLIDS.slice(0, 2) : SOLIDS;
...
/** מדידות אורך — המרות ס״מ/מ׳, השוואה וחיבור אורכים. */
function lengthMeasure(level = 1, p = {}) {
  ...
  if (lv === 3) {
    const a = rint(20, 80);
    let b = rint(20, 80);
    while (b === a) b = rint(20, 80);
    return {
      text: `קו אחד באורך ${a} ס״מ וקו שני ${b} ס״מ. מה האורך של שניהם ביחד?`,
      answer: a + b,
      difficulty: 3,
      hints: ["'ביחד' זה חיבור.", "שים/שימי לב שהיחידה זהה — שניהם בס״מ, אז אפשר לחבר ישר."],
      explanation: `${a} + ${b} = ${a + b} ס״מ.`,
    };
  }

  const m = rint(1, 4), extra = rint(10, 90);
  return {
    text: `לסרט יש ${m} מטר ועוד ${extra} ס״מ. כמה סנטימטרים בסך הכול?`,
    answer: m * 100 + extra,
    difficulty: 4,
