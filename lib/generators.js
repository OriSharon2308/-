...
function addition(level = 1, p = {}) {
  ...
  const difficulty = clamp(1 + Math.floor(answer / Math.max(5, max / 5)) + (carry ? 1 : 0), 1, 10);
  return {
    text: `${a} + ${b} = ?`,
    answer,
    difficulty,
...
function subtraction(level = 1, p = {}) {
  ...
  const difficulty = clamp(1 + Math.floor(a / Math.max(5, max / 5)) + (borrow ? 1 : 0), 1, 10);
  return {
    text: `${a} − ${b} = ?`,
    answer,
    difficulty,
...
function addSub100(level = 1, p = {}) {
  ...
  if (rint(0, 1) === 0) {
    ...
    return {
      text: `${a} + ${b} = ?`,
      answer,
      difficulty: clamp(2 + Math.floor(answer / 25), 1, 8),
  ...
  return {
    text: `${a} − ${b} = ?`,
    answer,
    difficulty: clamp(2 + Math.floor(a / 25), 1, 8),
  ...
  const extra = t.equal && t.right
    ...
    : t.equal
      ...
      : t.right
        ...
        : "";
  return {
    text: `צייר/י ${t.label} על הלוח${extra} — סמנ/י קודקודים, סגר/י את הצורה ולחצ/י "סיימתי".`,
    answer: t.label,
...
const CLOCK_KINDS = ["now", "next", "prev", "readmin", "set"];
...
function clock(level = 1) {
  ...
  // kind === "set" — אינטראקטיבי: הילד מזיז את המחוגים ולוחץ "סיימתי".
  const minute = SET_MINS[rint(0, SET_MINS.length - 1)];
  return {
    text: `כוון/י את השעון לשעה ${hour}:${pad2(minute)} — הזז/י את המחוגים ולחצ/י "סיימתי".`,
    answer: `${hour}:${pad2(minute)}`,
    answerKind: "time",
