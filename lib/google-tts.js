...
/** מפתח חשבון-השירות, אם הוגדר: JSON מלא / base64 / נתיב לקובץ. */
function saKey() {
...
/** JWT חתום RS256 → החלפה ל-access token מול oauth2.googleapis.com. בלי תלויות. */
      ...
      (res) => {
        ...
        res.on("end", () => {
          try {
            ...
            reject(new Error("SA token exchange: " + t.slice(0, 200)));
          } catch {
...
/** token טרי — קודם חשבון-שירות (עובד בענן), אחרת gcloud מקומי. */
function fetchFreshToken() {
  const sa = saKey();
  if (sa) return tokenFromServiceAccount(sa);
  return new Promise((resolve, reject) => {
    exec(
