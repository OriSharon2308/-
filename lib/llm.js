...
const tokenUsage = require("./token-usage"); // מעקב-טוקנים/עלות לכל תלמיד

// רישום הצריכה (טוקנים) של קריאה בודדת אל התלמיד — לעולם לא חוסם/מפיל את התשובה.
function recordUsage(useModel, usage, userId, label) {
  if (!userId || !usage) return;
  try { tokenUsage.record(userId, { model: useModel, usage, agent: label }); } catch { /* לא חוסם */ }
}
 ...
 */
async function complete({ system = "", messages = [], maxTokens = 1024, model: modelOverride = null, cacheSystem = false, userId = null, label = "" }) {
  ...
  recordUsage(useModel, u, userId, label);
 ...
 */
async function completeTools({ system = "", messages = [], tools = [], maxTokens = 1024, model: modelOverride = null, cacheSystem = false, userId = null, label = "" }) {
  if (!isEnabled()) throw new Error("AI disabled (no API key)");
  const useModel = (modelOverride && String(modelOverride).trim()) || model();
  const sys = buildSystemField(system, cacheSystem);
  const data = await httpsJson({
