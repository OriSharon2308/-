...
/**
 * יצירת הערכות ושמירה — רק לתפקידים שנתבקשו.
 *
 * עד היום נוצרו תמיד שלושתם יחד, וההורה שקרא רק את המורה שילם על שלוש
 * קריאות-מודל. עכשיו ברירת המחדל היא המורה בלבד, והשניים האחרים נוצרים
 * רק כשלוחצים עליהם בפועל.
 */
async function regenAll(userId, student, sig, roles = ROLES) {
  const want = roles.filter((r) => ROLES.includes(r));
  if (!want.length) return load(userId);
  const texts = await Promise.all(want.map((role) => generateOne(role, userId, student)));
  const data = load(userId);
  const now = new Date().toISOString();
  want.forEach((role, i) => { data[role] = { text: texts[i], hash: sig, updatedAt: now }; });
  save(userId, data);
  return data;
}
...
function queueRegen(userId, student, sig, roles) {
  if (inflight.has(userId)) return;
  inflight.add(userId);
  regenAll(userId, student, sig, roles)
...
async function getAssessments(userId, student, { force = false, roles = ["teacher"] } = {}) {
  let data = load(userId);
  const sig = inputSignature(userId);
  const want = roles.filter((r) => ROLES.includes(r));
  const missing = want.filter((r) => !(data[r] && data[r].text));
  // מתיישן = קיים אבל על נתונים ישנים. מרעננים ברקע רק מה שכבר נוצר פעם —
  // לא יוצרים תפקיד חדש שאיש לא ביקש.
  const staleExisting = ROLES.filter((r) => data[r] && data[r].text && data[r].hash !== sig);

  if (force || missing.length) {
    data = await regenAll(userId, student, sig, force ? want : missing);
  } else if (staleExisting.length) {
    queueRegen(userId, student, sig, staleExisting);
  }
  return {
...
async function generateTopicOne(role, student, t, minutes, userId = null) {
...
/** יצירת חוות-דעת לנושא — רק לתפקידים שנתבקשו; הקיימות נשמרות. */
async function regenTopic(userId, student, topicStat, minutes, sig, roles = ROLES) {
  const want = roles.filter((r) => ROLES.includes(r));
  const texts = await Promise.all(want.map((role) => generateTopicOne(role, student, topicStat, minutes, userId)));
  const data0 = load(userId);
  const prev = (data0.topics && data0.topics[topicStat.name]) || {};
  const out = { ...prev, hash: sig, updatedAt: new Date().toISOString() };
  want.forEach((role, i) => { out[role] = { text: texts[i] }; });
  const data = data0;
...
async function getTopicAssessments(userId, student, topicStat, minutes, { force = false, roles = ["teacher"] } = {}) {
  const data = load(userId);
  const sig = topicSignature(topicStat);
  const cached = (data.topics && data.topics[topicStat.name]) || null;
  const key = `${userId}::${topicStat.name}`;
  const want = roles.filter((r) => ROLES.includes(r));
  const missing = want.filter((r) => !(cached && cached[r] && cached[r].text));

  if (!force && !missing.length) {
    // רענון-רקע רק למה שכבר נוצר פעם — לא יוצרים תפקיד חדש בלי בקשה
    const staleExisting = ROLES.filter((r) => cached[r] && cached[r].text);
    if (cached.hash !== sig && !inflight.has(key)) {
      inflight.add(key);
      regenTopic(userId, student, topicStat, minutes, sig, staleExisting)
        .catch((e) => console.error("bg topic regen:", e.message))
        .finally(() => inflight.delete(key));
    }
    return { ...cached, fresh: false };
  }
  const out = await regenTopic(userId, student, topicStat, minutes, sig, force ? want : missing);
