...
/**
 * פילוח לשני האזורים של האתר — למידה מול תרגול.
 *
 * אירועי אזור-הלמידה נרשמים עם note="שיעור" (ראו server.js, סיום שלב בשיעור);
 * כל השאר הם תרגול. זה הסימן היחיד שמפריד ביניהם בציר-הזמן, ולכן הוא הבסיס
 * לכל מה שאזור ההורים מציג בנפרד לכל תחום.
 *
 * מחזיר לכל אזור: attempts/correct/accuracy, טווח תאריכים, מספר ימים פעילים,
 * ורשימת נושאים ממוינת לפי כמות — כל נושא עם הדיוק והפעילות האחרונה שלו *באזור הזה*.
 */
function areaBreakdown(userId) {
  const events = answeredEvents(userId);
  const blank = () => ({ attempts: 0, correct: 0, byTopic: new Map(), days: new Set(), last: null, first: null });
  const acc = { learn: blank(), practice: blank() };

  for (const e of events) {
    const a = acc[e.note === "שיעור" ? "learn" : "practice"];
    a.attempts += 1;
    if (e.correct) a.correct += 1;
    a.days.add(dayKey(e.t));
    if (!a.first || e.t < a.first) a.first = e.t;
    if (!a.last || e.t > a.last) a.last = e.t;
    const name = e.topic || "כללי";
    const t = a.byTopic.get(name) || { name, attempts: 0, correct: 0, last: null };
    t.attempts += 1;
    if (e.correct) t.correct += 1;
    if (!t.last || e.t > t.last) t.last = e.t;
    a.byTopic.set(name, t);
  }

  const shape = (a) => ({
    attempts: a.attempts,
    correct: a.correct,
    accuracy: a.attempts ? Math.round((a.correct / a.attempts) * 100) : 0,
    activeDays: a.days.size,
    firstActive: a.first,
    lastActive: a.last,
    topics: [...a.byTopic.values()]
      .map((t) => ({ ...t, accuracy: t.attempts ? Math.round((t.correct / t.attempts) * 100) : 0 }))
      .sort((x, y) => y.attempts - x.attempts),
  });

  return { learn: shape(acc.learn), practice: shape(acc.practice) };
}
...
function overview(allUsers) {
  ...
  for (const u of allUsers) {
    // דרך summary — סה"כ מהפרופיל (תרגול + למידה), לא רק מציר-הזמן. אחרת הסקירה מפספסת פעילות-למידה.
    const s = summary(u.id);
    totalAttempts += s.totalAttempts;
    totalCorrect += s.totalCorrect;
    if (s.lastActive === today) activeToday += 1;
  }
  return {
...
module.exports = { areaBreakdown, dailySeries, masteryByTopic, dailyTime, summary, overview, answeredEvents, activity, topicTime, timeOfDay, behaviorSignals, behaviorText };
