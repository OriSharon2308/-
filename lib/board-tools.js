...
const BOARD_TOOLS = [
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        label: { type: "string", description: "תווית אופציונלית (ברירת מחדל: shaded/parts). כשהשבר מלווה שאלה — העבר/י \"\" להסתרה, שהילד יגלה בעצמו" },
        color: { type: "string", description: "צבע אופציונלי" },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        label: { type: "string", description: "תווית אופציונלית (ברירת מחדל: rows × cols = מכפלה). כשהמערך מלווה שאלה — העבר/י \"\" להסתרה, שהילד יספור בעצמו" },
        color: { type: "string", description: "צבע אופציונלי" },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        label: { type: "string", description: "תווית אופציונלית (ברירת מחדל: הערך). כשהבלוקים מלווים שאלה — העבר/י \"\" להסתרה, שהילד יספור בעצמו" },
        color: { type: "string", description: "צבע אופציונלי" },
      },
      required: ["x", "y", "value"],
    },
  ...
  },
  {
    name: "draw_exercise",
    description:
      "תבנית מהירה (העדף/י אותה לכל תרגיל/שאלה!): מציירת תרגיל שלם עם תיבת-תשובה אינטראקטיבית בקריאה אחת, ומסדרת בשורות לבד. הכיוון והצד אוטומטיים לפי שפת הטקסט: משפט עברי → RTL, תיבה משמאל (סוף המשפט); ביטוי סימבולי → LTR, תיבה מימין. kind קובע רק את סוג בדיקת-התשובה (number/text). אל תצייר/י תרגילים ידנית. כמה שאלות — קריאה לכל אחת (אפשר בבת אחת).",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "money_coins",
    description:
      "כלי כסף (₪): הילד גורר מטבעות ושטרות (1,2,5,10,20,50 — בגדלים שונים לפי הערך) אל הארנק, והסכום מתעדכן חי. " +
      "לשאלות עלות/עודף/שארית: קבע/י target = הסכום הנכון שהילד צריך לבנות, ויש כפתור 'בדוק' שבודק אם הסכום נכון (ירוק/אדום). " +
      "לחיצה על מטבע בארנק מסירה אותו. בלי target → חקירה חופשית בלי בדיקה. את השאלה עצמה כתוב/י על הלוח/בצ'אט.",
    input_schema: {
      type: "object",
      properties: {
        target: { type: "integer", description: "הסכום הנכון בשקלים שהילד צריך לבנות לבדיקה (0/ריק = בלי בדיקה)" },
      },
      required: [],
    },
  },
  ...
  /* ── כלים לכיתות ג׳–ו׳ ──────────────────────────────────────────────
     עד כה לא היה כלי מוכן לחומר של ג׳–ו׳, וכל שיעור צייר HTML משלו ב-
     render_widget: אותו מושג נראה אחרת בכל פעם, והכל היה סטטי. הכלים
     האלה אינטראקטיביים ומדווחים הצלחה כמו שאר הערכה. */
  {
    name: "protractor",
    description:
      "מד-זווית אמיתי עם שני סולמות: הילד גורר את הקרן וקורא מעלות בקפיצות של 5°. " +
      "angle = הזווית ההתחלתית. target = הזווית שצריך להגיע אליה (הצלחה מסומנת בירוק).",
    input_schema: {
      type: "object",
      properties: {
        angle: { type: "integer", description: "זווית התחלה במעלות (0–180)" },
        target: { type: "integer", description: "הזווית שהילד צריך להגיע אליה (0–180)" },
        lock: { type: "boolean", description: "true = הקרן קבועה. חובה כששואלים 'כמה מעלות?'" },
        value: { type: "boolean", description: "false = בלי הקריאה המספרית, כדי לא להסגיר את התשובה" },
      },
      required: [],
    },
  },
  {
    name: "decimal_grid",
    description:
      "רשת עשרונית לחיצה: rows=1 → עשר עשיריות, rows=10 → מאה מאיות. הקריאה מציגה שבר ועשרוני זה לצד זה " +
      "(7/10 = 0.7), וזה בדיוק הגשר שהילד צריך. filled = כמה צבוע מראש, target = כמה צריך לצבוע.",
    input_schema: {
      type: "object",
      properties: {
        rows: { type: "integer", description: "1 לעשיריות, 10 למאיות" },
        filled: { type: "integer", description: "כמה משבצות צבועות מראש" },
        target: { type: "integer", description: "כמה משבצות צריך לצבוע" },
      },
      required: [],
    },
  },
  {
    name: "percent_bar",
    description:
      "פס-אחוזים נגרר: הילד מזיז ורואה בו-זמנית את האחוז ואת הכמות שהוא מייצג מתוך total. " +
      "כך '25% מ-60' מפסיק להיות נוסחה ונעשה דבר שרואים.",
    input_schema: {
      type: "object",
      properties: {
        total: { type: "integer", description: "הכמות השלמה (100%)" },
        percent: { type: "integer", description: "אחוז התחלתי (0–100)" },
        target: { type: "integer", description: "האחוז שצריך להגיע אליו" },
      },
      required: [],
    },
  },
  {
    name: "circle_parts",
    description:
      "מעגל עם רדיוס/קוטר/היקף — לחיצה מחליפה ביניהם ומראה את החישוב 3.14 × קוטר. " +
      "radius = הרדיוס במספרים של השאלה.",
    input_schema: {
      type: "object",
      properties: {
        radius: { type: "integer", description: "אורך הרדיוס" },
        show: { type: "string", description: "radius / diameter / circumference — מה מוצג ראשון" },
        lock: { type: "boolean", description: "true = בלי מחזור-לחיצה. חובה כשהמעגל הוא עזר לשאלה, אחרת הילד מגיע למצב שמציג את התשובה" },
      },
      required: ["radius"],
    },
  },
  {
    name: "place_value_table",
    description:
      "טבלת ערך-מקום עד מיליון: הספרות בעמודות עם שמות הבתים, ולחיצה על ספרה מגלה כמה היא *באמת* שווה " +
      "(ה-8 ב-586 היא 80). זו הטעות הכי נפוצה בערך-מקום.",
    input_schema: {
      type: "object",
      properties: {
        value: { type: "integer", description: "המספר שמוצג" },
        upto: { type: "integer", description: "כמה עמודות להציג (1–7)" },
      },
      required: ["value"],
    },
  },
  {
    name: "bar_chart",
    description:
      "דיאגרמת עמודות עם סולם מפורש ('כל משבצת = N') — כי שם ילדים נופלים. " +
      "series2 + name1/name2 הופכים אותה לדיאגרמה כפולה עם מקרא.",
    input_schema: {
      type: "object",
      properties: {
        labels: { type: "string", description: "שמות העמודות, מופרדים בפסיק" },
        series: { type: "string", description: "הערכים, מופרדים בפסיק" },
        series2: { type: "string", description: "סדרה שנייה (דיאגרמה כפולה)" },
        name1: { type: "string", description: "שם הסדרה הראשונה במקרא" },
        name2: { type: "string", description: "שם הסדרה השנייה במקרא" },
        step: { type: "integer", description: "כמה שווה משבצת אחת בסולם" },
        unit: { type: "string", description: '"squares" = כל פריט משבצת נפרדת (השיטה של ב׳–ג׳); ריק = עמודה מלאה' },
      },
  ...
  },
  {
    name: "area_grid",
    description:
      "מלבן על משבצות: הילד לוחץ ומונה שטח, וההיקף מוצג לצדו — כדי שההבדל בין 'בפנים' ל'מסביב' ייראה בעין.",
    input_schema: {
      type: "object",
      properties: {
        w: { type: "integer", description: "רוחב במשבצות (1–12)" },
        h: { type: "integer", description: "גובה במשבצות (1–8)" },
      },
      required: ["w", "h"],
    },
  },
  {
    name: "fraction_wall",
    description:
      "קיר-שברים: שורות של שלם, חצאים, שלישים… הילד לוחץ על חלקים ורואה כמה יוצא ביחד — " +
      "שוויון-ערך והשוואת שברים בעיניים, לפני הכלל.",
    input_schema: {
      type: "object",
      properties: {
        rows: { type: "string", description: "אילו מכנים להציג, מופרדים בפסיק (למשל 1,2,3,4,6,8)" },
      },
      required: [],
    },
  },
  {
    name: "solid_net",
    description:
      "גוף ופריסתו זה לצד זה — קובייה, תיבה, גליל, מנסרה משולשת או פירמידה. " +
      "כותרת הכלי מונה את הפאות, וזה מה שמחבר בין התלת-ממד לפריסה.",
    input_schema: {
      type: "object",
      properties: {
        solid: { type: "string", description: "cube / box / cylinder / prism / pyramid" },
      },
      required: ["solid"],
    },
  },

  {
    name: "remember_note",
    description:
      "הפנקס שלך (לא מצייר כלום): רשום/רשמי תובנה קצרה על הילד שתעזור לך בפעמים הבאות. " +
      "note — ההמשגה/הקושי במילים שלך (למשל \"מתבלבל במעבר עשרת — סופר באצבעות\"); " +
      "open_loop — מה נשאר פתוח להמשיך ממנו בשיעור הבא (למשל \"לא סגרנו מעבר-עשרת מעל 15\"). " +
      "השתמש/י כשאת/ה מזהה קושי אמיתי, וגם בסגירת שיעור אם משהו נשאר פתוח. הילד לא רואה את הפנקס.",
    input_schema: {
      type: "object",
      properties: {
        note: { type: "string", description: "תובנה קצרה על הקושי/ההמשגה (עד 160 תווים)" },
        open_loop: { type: "string", description: "מה להמשיך בשיעור הבא (עד 160 תווים)" },
        topic: { type: "string", description: "הנושא (ברירת מחדל: נושא השיעור הנוכחי)" },
      },
      required: [],
    },
  },
