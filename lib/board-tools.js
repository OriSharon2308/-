...
const BOARD_TOOLS = [
  ...
  // ── ערכת ווידג'טים אינטראקטיביים מוכנים (כל אחד = כרטיס במסגרת, נשלט-גודל). העדף/י אותם על render_widget כשהם מתאימים. ──
  {
    name: "interactive_fraction",
    description:
      "שבר אינטראקטיבי שהילד בונה: צורה מחולקת ל-parts חלקים שווים; הילד לוחץ על חלק כדי לצבוע/לבטל, ורואה n/parts מתעדכן. " +
      "לבניית שבר ע\"י הילד, שווי-ערך, השוואה. צורות: bar (מוט), circle (פיצה), grid (רשת). אם target>0 — בהשלמת המונה הנכון יש משוב הצלחה.",
    input_schema: {
      type: "object",
      properties: {
        ...
        shaded: { type: "integer", description: "כמה חלקים צבועים בהתחלה (0..parts, ברירת מחדל 0)" },
        target: { type: "integer", description: "מונה-יעד לבדיקה: כשהילד צובע בדיוק כך → הצלחה (0 = בלי בדיקה)" },
        label: { type: "string", description: "כותרת קצרה מעל השבר (אופציונלי)" },
      },
      required: ["parts"],
    },
  },
  {
    name: "count_objects",
    description:
      "אובייקטים מעוצבים לספירה בחיבור/חיסור (לא אימוג'י) — הילד סופר ולוחץ. left+right פריטים, פעולה op. למנייה, חיבור וחיסור קטנים.",
    input_schema: {
      type: "object",
      properties: {
        left: { type: "integer", description: "כמות בקבוצה הראשונה (0–10)" },
        right: { type: "integer", description: "כמות בקבוצה השנייה (0–10)" },
        op: { type: "string", enum: ["+", "-"], description: "פעולה: חיבור או חיסור (ברירת מחדל +)" },
  ...
  },
  {
    name: "ten_frame",
    description:
      "לוח-עשר (או כל גודל) למילוי: הילד לוחץ למלא/לרוקן משבצות. להשלמה ל-10, עשיריות, מבנה המספר. cells=מספר משבצות, perRow=בכל שורה.",
    input_schema: {
      type: "object",
      properties: {
        ...
        filled: { type: "integer", description: "כמה מלאות בהתחלה (ברירת מחדל 0)" },
        target: { type: "integer", description: "כמות-יעד למילוי לבדיקה (0 = בלי בדיקה)" },
      },
      required: ["cells"],
    },
  },
  {
    ...
    description:
      "בונה ערך-מקום בגרירה: הילד גורר יחידות/עשרות/מאות אל הלוח כדי לבנות את המספר target. לערך-מקום, פריטה והרכבת מספר. משוב כשמגיע ל-target.",
    input_schema: {
      type: "object",
      properties: {
        target: { type: "integer", description: "המספר לבנייה (1–999)" },
      },
      required: ["target"],
    },
  },
  {
    name: "mult_array",
    description:
      "מערך כפל אינטראקטיבי עד 10×10: הילד גורר ריבוע-בחירה (מתחיל משמאל-למטה) על רשת עיגולים ורואה rows×cols=מכפלה. לכפל כמערך וחוק החילוף. " +
      "maxRows/maxCols = גודל הרשת; targetRows/targetCols = הבחירה הנכונה לבדיקה.",
    input_schema: {
      type: "object",
      properties: {
        ...
        targetRows: { type: "integer", description: "שורות בבחירה הנכונה (לבדיקה)" },
        targetCols: { type: "integer", description: "עמודות בבחירה הנכונה (לבדיקה)" },
      },
  ...
  },
  {
    name: "mult_table",
    description:
      "לוח הכפל לחיץ עם מספרים מוסתרים שהילד ממלא. לשינון לוח-הכפל ובדיקת שליטה. max=גודל הלוח, hide=כמה תאים להעלים.",
    input_schema: {
      type: "object",
      properties: {
